...
def chk_status(log_loc = []):
	...
	for i in range(len(log_loc)):
		...
		filename="T_stamp_dir/T_stamp_"+str(start_string.group(1))
		...
		if T_val_chk != T_val:
			...
			print("Setting file pointers to default: "+log_loc[i]+": "+T_val)
			T_stamp_store.close()
			cmd="echo '0' > " + "offsets/"+str(start_string.group(1))+"_offset"
			os.system(cmd)
		else:
			pass

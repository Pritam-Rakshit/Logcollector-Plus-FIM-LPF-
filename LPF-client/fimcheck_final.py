			...
			for line in stat_file:
				if line != '':
					...
					if pattern: #Check and update hash
						...
						change_log=open("/var/log/fim_alert.log",'a+')
						if hashval != hashcheck[2].strip():
							change_log.write("File integrity changed"+"--"+str(hashcheck[0])+"--"+x+" --NEW Value: "+hashval+" --OLD VALUE: "+str(hashcheck[2]))
							for line in fileinput.FileInput("fim_db",inplace=1):
								sline=line.strip().split("--")
								if sline[0] != '' and line != "\n":
...
def FimInitiate(Interval, fimlocs = []):
	...
	while i > 0:
		if os.path.isfile(fimlocs[i-1]):
			...
			err_log=open("/var/LPF-client/error.log",'a')
			err_log.write("FIM location does not exist - "+fimlocs[i-1])
			err_log.close()
		i=i-1		
